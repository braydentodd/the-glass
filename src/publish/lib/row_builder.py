...
from src.publish.lib.column_structure import ColumnContext, _base_section
from src.publish.lib.formatters import (
    _format_companion,
    format_height,
    format_stat_value,
)
...
def build_entity_row(
    ...
    if section_data:
        ...
        if is_current:
            sec_ctx["seasons_in_query"] = 1
        primary_calculated = calculate_entity_stats(
            entity_data, entity_type, mode, sec_ctx
        )
    ...
    for entry in columns_list:
        ...
        if section_data and is_stats_section:
            ...
            row_section
            and is_stats_section
            and _base_section(col_ctx) != _base_section(row_section)
        ):
            ...
            pcts = (
                percentiles
                if not section_data
                else (section_data[first_section][1] if section_data else percentiles)
            )
        ...
        if not col_ctx_cfg.get("stats_timeframe"):
            use_entity = (
                sec_entity if section_data and is_stats_section else primary_entity
            )
            _col_mode = (
                col_ctx.rate
                if isinstance(col_ctx, ColumnContext) and col_ctx.rate
                else mode
            )
            value = evaluate_formula(
                col_key, use_entity, entity_type, _col_mode, context
            )
            if value is None:
                row.append("")
                ...
                formatted = format_stat_value(
                    value,
                    col_def.get("format", "number"),
                    col_def.get("decimal_places", 1),
        ...
        if col_def.get("is_opponent_col"):
            ...
            formatted = format_stat_value(
                value,
                active_def.get("format", "number"),
                active_def.get("decimal_places", 1),
                active_def.get("nullable", True),
            )
            row.append(formatted if formatted is not None else "")
            continue
...
def build_merged_entity_row(
    ...
    for rate_name in STAT_RATES:
        ...
        if current_data:
            section_data[
                ColumnContext(base_section="current_stats", rate=rate_name)
            ] = (current_data, rate_pcts.get("current_stats", {}), "")
            ...
            if isinstance(historical_data, dict):
                for y, h_data in historical_data.items():
                    if h_data:
                        section_data[
                            ColumnContext(
                                base_section="historical_stats",
                                timeframe=int(y),
                                rate=rate_name,
                            )
                        ] = (
                            h_data,
                            rate_pcts.get(f"historical_stats_{y}yr", {}),
                            str(y),
                        )
            else:
                section_data[
                    ColumnContext(base_section="historical_stats", rate=rate_name)
                ] = (
                    historical_data,
                    rate_pcts.get("historical_stats", {}),
            ...
            if isinstance(postseason_data, dict):
                for y, p_data in postseason_data.items():
                    if p_data:
                        section_data[
                            ColumnContext(
                                base_section="postseason_stats",
                                timeframe=int(y),
                                rate=rate_name,
                            )
                        ] = (
                            p_data,
                            rate_pcts.get(f"postseason_stats_{y}yr", {}),
                            str(y),
                        )
            else:
                section_data[
                    ColumnContext(base_section="postseason_stats", rate=rate_name)
                ] = (
                    postseason_data,
                    rate_pcts.get("postseason_stats", {}),
                    post_seasons,
                )
    ...
    for col_idx, entry in enumerate(columns_list):
        ...
        if is_stats_section:
            ...
            if isinstance(col_ctx, ColumnContext):
                sec_mode = col_ctx.rate if col_ctx.rate else "per_poss"
            else:
                sec_mode = (
                    col_ctx.split("__")[1]
                    if col_ctx and "__" in str(col_ctx)
                    else "per_poss"
                )
            ...
            if col_def.get("is_opponent_col") and opp_percentiles:
                ...
                if opp_pop is not None:
                    ...
                    if base_col_def:
                        opp_fn = (
                            base_col_def.get("values", {}).get("team", {}).get("fn")
                        )
                        try:
                            value = (
                                opp_fn(sec_entity, None) if callable(opp_fn) else None
                            )
                        except (TypeError, ZeroDivisionError, KeyError):
                            value = None
                        if value is not None:
                            ...
                            median = _get_value_at_percentile(
                                opp_pop, 50, reverse=False
                            )
                            diff = value - median if median is not None else None
            ...
            calculated = calculate_entity_stats(
                sec_entity, entity_type, sec_mode, sec_ctx_pct
            )
            value = calculated.get(base_key)
            ...
            default_current_key = ColumnContext(
                base_section="current_stats", rate="per_poss"
            )
            ...
            base_def = VIEW_COLUMNS.get(base_key, col_def)
            calculated = calculate_entity_stats(
                sec_entity, entity_type, "per_poss", context
            )
            value = calculated.get(base_key)
...
def _get_value_at_percentile(
    sorted_values: List, percentile: float, reverse: bool = False
) -> Any:
    ...
    if weights:
        ...
        if not isinstance(v_lower, (int, float)) or not isinstance(
            v_upper, (int, float)
        ):
            return None
...
def build_summary_rows(
    ...
    for label, pct_level in SUMMARY_THRESHOLDS:
        ...
        for col_idx, entry in enumerate(columns_list):
            ...
            if col_def.get("is_generated_percentile", False):
                ...
                if col_def.get("is_opponent_col") and opp_percentiles:
                    if is_stats_section and col_ctx:
                        opp_pop = opp_percentiles.get(base_key, {}).get(
                            _base_section(col_ctx)
                        )
                        if opp_pop:
                            reverse = base_def.get("percentile") == "reverse"
                            value = _get_value_at_percentile(
                                opp_pop, pct_level, reverse
                            )
                            median = _get_value_at_percentile(opp_pop, 50, False)
                elif is_stats_section:
                    pop_key = f"{col_ctx}:{base_key}"
                    sorted_vals = percentile_pops.get(
                        pop_key, percentile_pops.get(base_key)
                    )
                    if sorted_vals:
                        reverse = base_def.get("percentile") == "reverse"
                        value = _get_value_at_percentile(
                            sorted_vals, pct_level, reverse
                        )
                        median = _get_value_at_percentile(
                            sorted_vals, 50, reverse=False
                        )
                else:
                ...
                diff = (
                    value - median if value is not None and median is not None else None
                )
            ...
            if not col_def.get("percentile") and not col_def.get(
                "is_generated_percentile", False
            ):
                row.append("")
                continue
            ...
            if col_def.get("is_opponent_col") and opp_percentiles:
                ...
                if col_ctx_cfg.get("stats_timeframe") and col_ctx:
                    opp_pop = opp_percentiles.get(col_key, {}).get(
                        _base_section(col_ctx)
                    )
                    if opp_pop:
