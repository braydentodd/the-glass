...
Pure color interpolation math - destination-agnostic.
"""

from typing import Dict

from src.publish.definitions.presentation import COLORS, COLOR_THRESHOLDS
...
def get_color_for_percentile(percentile: float, reverse: bool = False) -> Dict:
    ...
    Args:
        ...
        reverse: If True, invert the percentile before computing color
...
def get_color_dict(color_name: str) -> Dict:
    ...
    Args:
        color_name: Key in COLORS dict
        
    Returns:
        Color dict with RGB values
    """
    return COLORS.get(color_name, COLORS['white'])


def get_color_for_raw(color_dict: Dict) -> Dict:
    """Ensure a color dict has the right keys for destination API.
    
    Args:
        color_dict: Dict that may or may not have all RGB keys
        
    Returns:
        Dict with 'red', 'green', 'blue' keys (defaults to 0)
    """
    return {
